...
export type FilterSortedByTypes = 'name' | 'artist' | 'album_name'
...
export type FiltersSelectorValuesTypes<T extends FilterSortedByTypes> = T extends 'name' ? FilterSongNameTypes | FilterSongNameTypes[] : T extends 'artist' ? string | string[] : T extends 'album_name' ? FilterAlbumNameTypes | FilterAlbumNameTypes[] : never

export interface FilterOptionsWithSelector<T extends NonNullable<FilterSortedByTypes>> extends ApplyFiltersOnlyOptions {
  /**
...
export type FilterOptionsType<T extends FilterSortedByTypes> = T extends null ? ApplyFiltersOnlyOptions : FilterOptionsWithSelector<NonNullable<T>>
 ...
 * @returns {DTAFile[]} A filtered array of parsed songs.
 */
export const filterDTA = <T extends FilterSortedByTypes, V extends FilterOptionsType<T>>(songs: DTAFile[], filterSelector: T, filters: V): DTAFile[] => {
  let returnValue = sortDTA(songs, 'name')

  if (filterSelector !== null) {
    const options = filters as FilterOptionsWithSelector<typeof filterSelector>

    if (filterSelector === 'name' && !Array.isArray(options.filterSelectorValues)) {
      const filterSelectorValues = options.filterSelectorValues as FilterSongNameTypes

      returnValue = returnValue.filter((song) => {
        if (filterSelectorValues === '123' && /^[^a-zA-Z]/.test(song.get('name'))) return song
        else if (song.get('name', { leadingArticle: 'omit' }).slice(0, 1).toLowerCase() === filterSelectorValues.toLowerCase()) return song
      })
    } else if (filterSelector === 'name' && Array.isArray(options.filterSelectorValues)) {
      const filterSelectorValues = options.filterSelectorValues as FilterSongNameTypes[]
    ...
    } else if (filterSelector === 'artist' && !Array.isArray(options.filterSelectorValues)) {
      const filterSelectorValues = options.filterSelectorValues as string

      returnValue = returnValue.filter((song) => {
        if (song.content.artist.toLowerCase() === filterSelectorValues.toString().toLowerCase()) return song
      })
    } else if (filterSelector === 'artist' && Array.isArray(options.filterSelectorValues)) {
      const filterSelectorValues = options.filterSelectorValues as string[]
    ...
    } else if (filterSelector === 'album_name' && !Array.isArray(options.filterSelectorValues)) {
      const { album_name, artist } = options.filterSelectorValues as FilterAlbumNameTypes

      returnValue = returnValue.filter((song) => {
        if (song.content.artist.toLowerCase() === artist.toLowerCase() && song.content.album_name?.toLowerCase() === album_name.toLowerCase()) return song
      })
    } else if (filterSelector === 'album_name' && Array.isArray(options.filterSelectorValues)) {
      const filterSelectorValues = options.filterSelectorValues as FilterAlbumNameTypes[]
      ...
      returnValue = returnValue.filter((song) => {
        ...
        filterSelectorValues.forEach((filters) => {
          ...
          if (song.content.artist.toLowerCase() === artist.toLowerCase() && song.content.album_name?.toLowerCase() === album_name.toLowerCase()) proof = true
        })
 ...
 * @returns {DTAFile[]} A filtered array of parsed songs.
 */
export const applyFilters = (songs: DTAFile[], filters: ApplyFiltersOnlyOptions): DTAFile[] => {
  if (filters.genres) {
