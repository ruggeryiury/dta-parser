...
import { genTabs } from '../utils/genTabs'
import { trackCountStringGenerator } from '../utils/trackCountStringGenerator'
 ...
 * - - - -
 * @param {DTAFile[] | DTAFileContents[] | DTAFile | DTAFileContents} songs An array of parsed song objects.
 ...
 */
export const stringifyDTA = (songs: DTAFile[] | DTAFileContents[] | DTAFile | DTAFileContents, options?: StringifyDataOptions): string => {
  ...
  if (Array.isArray(songs)) {
    songs.forEach((value) => {
      if ('content' in value) {
        ...
        const [converted] = JSONtoDTA(value)
        if (type === 'rb3_dlc') output += stringifyRB3DLC(converted, options as StringifyDataOptions)
        else output += stringifyDefault(converted, options as StringifyDataOptions)
      }
    })
  } else {
    if ('content' in songs) {
      ...
      const [converted] = JSONtoDTA(songs)
      if (type === 'rb3_dlc') output += stringifyRB3DLC(converted, options)
      else output += stringifyDefault(converted, options)
    }
  }
...
const stringifyDefault = (value: DTAFile, options: StringifyDataOptions): string => {
  ...
  output += `(${genTabs()}'${id}'${genTabs()}(${genTabs(2)}'name'${genTabs(2)}"${name.replace(/"/g, '\\q')}"${genTabs()})${genTabs()}(${genTabs(2)}'artist'${genTabs(2)}"${artist.replace(/"/g, '\\q')}"${genTabs()})${genTabs()}('master' ${master ? 1 : 0})${context ? `${genTabs()}('context' ${context})` : ''}${genTabs()}(${genTabs(2)}'song'${genTabs(2)}(${genTabs(3)}'name'${genTabs(3)}"${wiiMode ? `dlc/${wiiMode.gen}/${wiiMode.slot.toString().padStart(3, '0')}/content/songs/${songname}/${songname}` : `songs/${songname}/${songname}`}"${genTabs(2)})${genTabs(2)}(${genTabs(3)}'tracks_count'${genTabs(3)}(${tracks_count.join(' ')})${genTabs(2)})${genTabs(2)}(${genTabs(3)}'tracks'${genTabs(3)}(${genTabs(4)}`
  ...
  output += `${crowd ? `${genTabs(2)}('crowd_channels' ${trackCountStringGenerator(crowdTrackStarts, 2)})` : ``}${genTabs(2)}('vocal_parts' ${vocal_parts === undefined ? (vocals > 0 ? 1 : 0) : vocal_parts})${genTabs(2)}(${genTabs(3)}'drum_solo'${genTabs(3)}(${genTabs(4)}'seqs'${genTabs(4)}('kick.cue' 'snare.cue' 'tom1.cue' 'tom2.cue' 'crash.cue')${genTabs(3)})${genTabs(2)})${genTabs(2)}(${genTabs(3)}'drum_freestyle'${genTabs(3)}(${genTabs(4)}'seqs'${genTabs(4)}('kick.cue' 'snare.cue' 'hat.cue' 'ride.cue' 'crash.cue')${genTabs(3)})${genTabs(2)})${genTabs(2)}(mute_volume ${mute_volume === undefined ? '-96' : mute_volume})${genTabs(2)}(mute_volume_vocals ${mute_volume_vocals === undefined ? '-12' : mute_volume_vocals})${genTabs(2)}(hopo_threshold ${hopo_threshold === undefined ? '170' : hopo_threshold})${genTabs()})${genTabs()}('song_scroll_speed' ${song_scroll_speed === undefined ? '2300' : song_scroll_speed})${genTabs()}(${genTabs(2)}'bank'${genTabs(2)}"${bank}"${genTabs()})${genTabs()}(drum_bank ${drum_bank})${genTabs()}('anim_tempo' ${anim_tempo})${band_fail_cue === undefined ? '' : `${genTabs()}(band_fail_cue ${band_fail_cue})`}${genTabs()}('song_length' ${song_length})${genTabs()}('preview' ${preview.join(' ')})${genTabs()}(${genTabs(2)}'rank'`
  ...
  output += `${genTabs(2)}('band' ${rank_band})${genTabs()})${genTabs()}('genre' '${genre === undefined ? 'other' : genre}')${genTabs()}('vocal_gender' '${vocal_gender}')${genTabs()}('version' ${version === undefined ? 30 : version})${genTabs()}('format' ${format === undefined ? 10 : format})${genTabs()}('album_art' ${album_art ? 1 : 0})${genTabs()}('year_released' ${year_released})${year_recorded === undefined ? '' : `${genTabs()}('year_recorded' ${year_recorded})`}${genTabs()}('rating' ${rating === undefined ? 4 : rating})${sub_genre ? `${genTabs()}('sub_genre' '${sub_genre}')` : ''}${genTabs()}('song_id' ${song_id})`
  ...
  output += `${genTabs()}('tuning_offset_cents' ${tuning_offset_cents === undefined ? 0 : tuning_offset_cents})${genTabs()}('guide_pitch_volume' ${guide_pitch_volume === undefined ? '-3.00' : guide_pitch_volume})${genTabs()}('game_origin' '${game_origin === undefined ? 'ugc_plus' : game_origin}')${genTabs()}('encoding' '${encoding === undefined ? 'latin1' : encoding}')${album_name ? `${genTabs()}(${genTabs(2)}'album_name'${genTabs(2)}"${album_name.replace(/"/g, '\\q')}"${genTabs()})` : ''}${album_track_number ? `${genTabs()}('album_track_number' ${album_track_number})` : ''}${vocal_tonic_note === undefined ? '' : `${genTabs()}(vocal_tonic_note ${vocal_tonic_note})`}${song_tonality === undefined ? '' : `${genTabs()}(song_tonality ${song_tonality})`}${song_key === undefined ? '' : `${genTabs()}(song_key ${song_key})`}${real_guitar_tuning ? `${genTabs()}(real_guitar_tuning (${real_guitar_tuning.join(' ')}))` : ''}${real_bass_tuning ? `${genTabs()}(real_bass_tuning (${real_bass_tuning.join(' ')}))` : ''}${pack_name ? `${genTabs(1)}(pack_name "${pack_name.replace(/"/g, '\\q')}")` : ''}${base_points ? `${genTabs(1)}(base_points ${base_points})` : ''}`

  if (placeCustomAttributes === undefined || placeCustomAttributes === true) {
    output += `${genTabs()}${genTabs(0)};DO NOT EDIT THE FOLLOWING LINES MANUALLY${genTabs(0)};Created using Magma: C3 Roks Edition v3.3.5${genTabs(0)};Song authored by ${author ? author : 'Unknown Charter'}${genTabs(0)};Song=${name}${genTabs(0)};Language(s)=${languages ? (languages.length === 1 ? `${languages[0]},` : `${languages.join(',')},`) : `English,`}${genTabs(0)};Karaoke=${karaoke ? 1 : 0}${genTabs(0)};Multitrack=${multitrack ? 1 : 0}${genTabs(0)};Convert=${convert ? 1 : 0}${genTabs(0)};2xBass=${doubleKick ? 1 : 0}${genTabs(0)};RhythmKeys=${rhythmOnKeys ? 1 : 0}${genTabs(0)};RhythmBass=${rhythmOnBass ? 1 : 0}${genTabs(0)};CATemh=${CATemh ? 1 : 0}${genTabs(0)};ExpertOnly=${expertOnly ? 1 : 0}`
  }
...
const stringifyRB3DLC = (value: DTAFile, options: StringifyDataOptions): string => {
  ...
  output += `${genTabs(2)}(band ${rank_band})${genTabs()})${genTabs()}(format ${format === undefined ? 10 : format})${genTabs()}(version ${version === undefined ? 30 : version})${genTabs()}(game_origin ${game_origin === undefined ? 'ugc_plus' : game_origin})${genTabs()}(rating ${rating === undefined ? 4 : rating})${genTabs()}(genre ${genre === undefined ? 'other' : genre})${genTabs()}(sub_genre ${sub_genre === undefined ? 'subgenre_other' : sub_genre})${genTabs()}(vocal_gender ${vocal_gender})${genTabs()}(year_released ${year_released})${year_recorded ? `${genTabs()}(year_recorded ${year_recorded})` : ``}${genTabs()}(album_art ${album_art ? 'TRUE' : 'FALSE'})${album_name ? `${genTabs()}(album_name "${album_name.replace(/"/g, '\\q')}")` : ``}${album_track_number ? `${genTabs()}(album_track_number ${album_track_number})` : ``}${vocal_tonic_note === undefined ? `` : `${genTabs()}(vocal_tonic_note ${vocal_tonic_note})`}${song_tonality === undefined ? `` : `${genTabs()}(song_tonality ${song_tonality})`}${song_key === undefined ? `` : `${genTabs()}(song_key ${song_key})`}${tuning_offset_cents === undefined ? `` : `${genTabs()}(tuning_offset_cents ${tuning_offset_cents})`}${guide_pitch_volume === undefined ? `` : `${genTabs()}(guide_pitch_volume ${guide_pitch_volume.toFixed(1)})`}${real_guitar_tuning ? `${genTabs()}(real_guitar_tuning (${real_guitar_tuning.join(' ')}))` : ``}${real_bass_tuning ? `${genTabs()}(real_bass_tuning (${real_bass_tuning.join(' ')}))` : ``}${encoding === undefined || encoding === 'latin1' ? `` : `${genTabs()}(encoding ${encoding})`}${pack_name ? `${genTabs()}(pack_name "${pack_name.replace(/"/g, '\\q')}")` : ``}${base_points ? `${genTabs()}(base_points ${base_points})` : ``}`
