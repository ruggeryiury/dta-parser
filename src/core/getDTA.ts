...
import { AnimTempoValues, BandFailCueValues, BandRankingsOptions, BankValues, DrumBankValues, GenreValues, InstrumentRankingsOptions, Locale, RankTypes, RatingValues, SongKeyMajorValues, SongKeyMinorValues, SongScrollSpeedValues, SpotifyAlbumSearchDocument, SubGenreValues, VocalGenderValues, VocalPartsValues, getAlbumArt, leadingArticle2Trailing, millisecondsToSeconds, millisecondsToTimeString, omitLeadingArticle, rankCalculator } from '../exports'

export type GetDataValueTypes = keyof Omit<DTAContentDocument, 'tracks_count' | 'pans' | 'vols' | 'preview' | 'solo' | 'guide_pitch_volume' | 'encoding' | 'format' | 'version' | 'game_origin' | 'vocal_tonic_note' | 'song_tonality' | 'languages'>
export type GetDataValueOptions<V extends GetDataValueTypes> = V extends 'name' | 'artist' | 'album_name'
    ? GetDataNamingOptions
    : V extends 'vocal_parts' | 'bank' | 'drum_bank' | 'anim_tempo' | 'band_fail_cue' | 'song_scroll_speed' | 'rating' | 'genre' | 'mute_volume' | 'mute_volume_vocals' | 'tuning_offset_cents' | 'sub_genre' | 'vocal_gender'
    ? GetDataRawOptions
    : V extends 'song_length'
    ? GetDataTimeOptions
    : V extends 'rank_band' | 'rank_drum' | 'rank_bass' | 'rank_guitar' | 'rank_vocals' | 'rank_keys' | 'rank_real_keys' | 'rank_real_guitar' | 'rank_real_bass'
    ? GetDataRankingOptions
    : V extends 'album_art'
    ? GetDataAlbumArtOptions
    : never

export type GetDataValueReturn<V extends GetDataValueTypes, O extends GetDataValueOptions<V>> = V extends 'vocal_parts'
    ? O extends { raw: true }
 ...
 */
export const getDTA = <V extends GetDataValueTypes, O extends GetDataValueOptions<V>>(dta: DTADocument, value: V, options?: O): GetDataValueReturn<V, O> => {
    if (value === 'name' || value === 'artist' || value === 'album_name') {
        ...
        if (leadingArticle === undefined || leadingArticle === 'emit') returnValue = dta.content[value as keyof DTAContentDocument] as string
        else if (leadingArticle === 'omit') returnValue = omitLeadingArticle(dta.content[value as keyof DTAContentDocument] as string)
        else returnValue = leadingArticle2Trailing(dta.content[value as keyof DTAContentDocument] as string)
        ...
        if (raw !== undefined || raw) return dta.content[value as keyof DTAContentDocument] as GetDataValueReturn<V, O>
            ...
            if (value === 'vocal_parts') return Locale.vocal_parts(dta.content.vocal_parts === undefined ? 0 : dta.content.vocal_parts) as GetDataValueReturn<V, O>
        ...
        if (returnType === 'verbosed') return Locale.rank(rank) as GetDataValueReturn<V, O>
        else if (returnType === 'raw') return operators[1] as GetDataValueReturn<V, O>
        else if (returnType === 'graphical') return Locale.rank(rank, 'graphical') as GetDataValueReturn<V, O>
        else return rank as GetDataValueReturn<V, O>
    } else if (value === 'song_key') {
        const key = dta.content.song_key !== undefined ? dta.content.song_key : dta.content.vocal_tonic_note !== undefined ? dta.content.vocal_tonic_note : -1
        const tonality = dta.content.song_tonality !== undefined ? dta.content.song_tonality : -1
