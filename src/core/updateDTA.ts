...
import { AnimTempoValues, BandFailCueValues, BankValues, DrumBankValues, GenreValues, RatingValues, SongKeyMajorValues, SongKeyMinorValues, SongScrollSpeedValues, TrainerKeyOverrideValues, VocalGenderValues, VocalPartsValues, bandRankCalculator, dtaRankCalculator, getKeyFromValue, pansArrayGenerator, timeStringToMilliseconds } from '../exports'
...
export type BandRankingsOptions = 0 | 'Warmup' | 1 | 'Apprentice' | 2 | 'Solid' | 3 | 'Moderate' | 4 | 'Challenging' | 5 | 'Nightmare' | 6 | 'Impossible'
...
export interface DrumUpdateOptions extends Omit<InstrumentPartRequiredUpdateOptions, 'channels'> {
    /**
...
export interface GuitarUpdateOptions extends InstrumentPartRequiredUpdateOptions {
    /**
...
export interface VocalsUpdateOptions extends InstrumentPartRequiredUpdateOptions {
    /**
...
export type SubGenreUpdateValues<G extends GenreValues> = G extends 'Classical' | 'Classic Rock' | 'Emo' | 'Fusion' | 'Grunge' | 'Inspirational' | 'J-Rock' | 'Latin' | 'Novelty' | 'Nu-Metal' | 'Southern Rock' | 'World'
    ? G
...
export const updateDTA = (dta: DTADocument, update: UpdateDataOptions) => {
    const { id, name, artist, master, song_id, songname, tracks, mute_volume, mute_volume_vocals, hopo_threshold, bank, drum_bank, anim_tempo, band_fail_cue, song_scroll_speed, preview, song_length, rank_band, game_origin, rating, genre, year_released, year_recorded, album, key_signature, pack_name, author, karaoke, multitrack, doubleKick, convert, rhythmOnKeys, rhythmOnBass, CATemh, expertOnly } = update
    ...
    if (tracks) {
        ...
        const backingT = tracks.backing ? pansArrayGenerator(tracks.backing).length : 0
    ...
    if (anim_tempo) dta.content.anim_tempo = getKeyFromValue.anim_tempo(anim_tempo)
    ...
    if (key_signature) {
        ...
        if (key_signature.trainer_key_override) {
            ...
            if (key_signature.trainer_key_override === 'C♯/D♭') dta.content.song_key = 1
            if (key_signature.trainer_key_override === 'D') dta.content.song_key = 2
            if (key_signature.trainer_key_override === 'D♯/E♭') dta.content.song_key = 3
            ...
            if (key_signature.trainer_key_override === 'F♯/G♭') dta.content.song_key = 6
            if (key_signature.trainer_key_override === 'G') dta.content.song_key = 7
            if (key_signature.trainer_key_override === 'G♯/A♭') dta.content.song_key = 8
            if (key_signature.trainer_key_override === 'A') dta.content.song_key = 9
            if (key_signature.trainer_key_override === 'A♯/A♭') dta.content.song_key = 10
