import { localeValueToKey, type AnimTempo, type BandFailCueNames, type BandRankingNames, type BandRankingNumbers, type DrumBankNames, type DTAFile, type PartialDTAFile, type PercussionBankNames, type SongEncoding, type SongGenreNames, type SongRating, type SongRatingNames, type SongScrollSpeedNames, type VocalGenderNames, type VocalParts, type VocalPartsNames } from '../core.js'
...
export interface DTAUpdateOptions {
  /**
...
export type DTAUpdateOptionsForExtend = Partial<DTAUpdateOptions>
export type SongUpdateObject = Record<string, DTAUpdateOptions>
 ...
 */
export const updateDTA = (dta: PartialDTAFile, update: DTAUpdateOptionsForExtend): PartialDTAFile => {
  const newDTA = {
    ...dta,
  } as PartialDTAFile
  ...
  if (double_kick !== undefined) newDTA.double_kick = double_kick

  if (convert !== undefined) newDTA.convert = convert

  if (rhythm_on_bass !== undefined) newDTA.rhythm_on_bass = rhythm_on_bass

  if (rhythm_on_keys !== undefined) newDTA.rhythm_on_keys = rhythm_on_keys

  if (cat_ehm !== undefined) newDTA.cat_ehm = cat_ehm

  if (expert_only !== undefined) newDTA.expert_only = expert_only

  return newDTA as DTAFile
}

export interface DTAFileWithUpdates extends DTAFile {
  update?: DTAUpdateOptions
}

export interface PartialDTAFileWithUpdates extends PartialDTAFile {
  update?: DTAUpdateOptions
}
 ...
 */
export const applyUpdatesToDTAFileObject = (song: PartialDTAFileWithUpdates): PartialDTAFile => {
  if (song.update) {
    const newDTA = {
      ...song,
