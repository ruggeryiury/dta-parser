...
const stringifyDTADocumentC3 = (value: DTADocument, options?: StringifyDataOptions): string => {
    let output = ''
    let trackCount = 0
    ...
    if ((options?.placeCustomAttributes === undefined || options?.placeCustomAttributes === true) && value.custom) {
        output += `\n;DO NOT EDIT THE FOLLOWING LINES MANUALLY\n;Created using Magma: C3 Roks Edition v3.3.5\n;Song authored by ${value.custom.author ? value.custom.author : 'Unknown Charter'}\n;Song=${value.content.name}\n;Language(s)=${value.custom.languages ? (value.custom.languages.length === 1 ? `${value.custom.languages.join(',')},` : value.custom.languages.join(',')) : 'English,'}\n;Karaoke=${value.custom.karaoke ? 1 : 0}\n;Multitrack=${value.custom.multitrack ? 1 : 0}\n;Convert=${value.custom.convert ? 1 : 0}\n;2xBass=${value.custom.doubleKick ? 1 : 0}\n;RhythmKeys=${value.custom.rhythmOnKeys ? 1 : 0}\n;RhythmBass=${value.custom.rhythmOnBass ? 1 : 0}\n;CATemh=${value.custom.CATemh ? 1 : 0}\n;ExpertOnly=${value.custom.expertOnly ? 1 : 0}\n`
    }
    output += `)\n`
    return output
}

const stringifyDTADocumentRb3 = (value: DTADocument, options?: StringifyDataOptions): string => {
    let output = ''
    let trackCount = 0
    ...
    if (drumTrack) {
        output += `${!firstTrackDone ? '' : '\n\t\t\t\t'}(drum ${drumTrack > 1 ? '(' : ''}${trackCountIterator(trackCount, value.content.tracks_count[0])}${drumTrack > 1 ? ')' : ''})`
        trackCount += value.content.tracks_count[0]
        firstTrackDone = true
    }
    if (bassTrack) {
        output += `${!firstTrackDone ? '' : '\n\t\t\t\t'}(bass ${bassTrack > 1 ? '(' : ''}${trackCountIterator(trackCount, value.content.tracks_count[1])}${bassTrack > 1 ? ')' : ''})`
        trackCount += value.content.tracks_count[1]
        firstTrackDone = true
    }
    if (guitarTrack) {
        output += `${!firstTrackDone ? '' : '\n\t\t\t\t'}(guitar ${guitarTrack > 1 ? '(' : ''}${trackCountIterator(trackCount, value.content.tracks_count[2])}${guitarTrack > 1 ? ')' : ''})`
        trackCount += value.content.tracks_count[2]
        firstTrackDone = true
    }
    if (vocalsTrack) {
        output += `${!firstTrackDone ? '' : '\n\t\t\t\t'}(vocals ${vocalsTrack > 1 ? '(' : ''}${trackCountIterator(trackCount, value.content.tracks_count[3])}${vocalsTrack > 1 ? ')' : ''})`
        trackCount += value.content.tracks_count[3]
        firstTrackDone = true
    }
    if (keysTrack) {
        output += `${!firstTrackDone ? '' : '\n\t\t\t\t'}(keys ${keysTrack > 1 ? '(' : ''}${trackCountIterator(trackCount, value.content.tracks_count[4])}${keysTrack > 1 ? ')' : ''})`
        trackCount += value.content.tracks_count[4]
        firstTrackDone = true
    }
    output += `))\n`
...
export const stringifyDTA = (songs: DTADocument[], options?: StringifyDataOptions): string => {
    ...
    songs.forEach((value) => {
        if (options?.type === 'rb3') {
            ...
            output += stringifyDTADocumentC3(value, options).replace(/\t/g, '   ')
            return
        }
    })
