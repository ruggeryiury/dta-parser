 ...
 */
export const getDTA = <V extends GetDataValueTypes, O extends GetDataValueOptions<V>>(
    dta: DTADocument,
    value: V,
    options?: O
    ...
    if (value === 'name' || value === 'artist' || value === 'album_name') {
        ...
        if (raw !== undefined || raw) return dta.content[value as keyof DTAContentDocument] as GetDataValueReturn<V, O>
            ...
            if (value === 'vocal_parts') return Locale.vocal_parts(dta.content.vocal_parts) as GetDataValueReturn<V, O>
            ...
            else if (value === 'drum_bank') return Locale.drum_bank(dta.content.drum_bank) as GetDataValueReturn<V, O>
            else if (value === 'anim_tempo') return Locale.anim_tempo(dta.content.anim_tempo) as GetDataValueReturn<V, O>
                ...
                return `${dta.content.tuning_offset_cents === undefined ? '0' : dta.content.tuning_offset_cents} cents` as GetDataValueReturn<V, O>
            else if (value === 'rating') return Locale.rating(dta.content.rating) as GetDataValueReturn<V, O>
            else if (value === 'genre') return Locale.genre(dta.content.genre) as GetDataValueReturn<V, O>
            else if (value === 'sub_genre') return Locale.sub_genre(dta.content.sub_genre) as GetDataValueReturn<V, O>
            else if (value === 'vocal_gender') return Locale.vocal_gender(dta.content.vocal_gender) as GetDataValueReturn<V, O>
            else return Locale.song_scroll_speed(dta.content.song_scroll_speed) as GetDataValueReturn<V, O>
        }
        ...
        if ((options as GetDataTimeOptions).format === undefined || (options as GetDataTimeOptions).format === 'formatted')
            return millisecondsToTimeString(dta.content.song_length) as GetDataValueReturn<V, O>
        ...
        const key =
            dta.content.song_key !== undefined ? dta.content.song_key : dta.content.vocal_tonic_note !== undefined ? dta.content.vocal_tonic_note : -1
 ...
 */
export const getSongIndexByID = (songs: DTADocument[], id: string): number | undefined => {
