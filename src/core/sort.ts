...
export type SongSortingTypes =
  | (typeof sortingLocale)[keyof typeof sortingLocale]
  | null
...
export const sortDTA = (
  ...
  if (sortBy === 'Song Title') {
    ...
    return songs.sort((a, b) => {
      if (
        omitLeadingArticle(a.artist.toLowerCase()) >
        omitLeadingArticle(b.artist.toLowerCase())
      )
        return 1
      if (
        omitLeadingArticle(a.artist.toLowerCase()) <
    ...
    sortBy === 'Artist -> Year Released -> Album Name -> Album Track Number'
  ) {
    return songs.sort((a, b) => {
      if (
        omitLeadingArticle(a.artist.toLowerCase()) >
      ...
      if (
        a.album_name &&
        b.album_name &&
        a.album_name.toLowerCase() > b.album_name.toLowerCase()
      )
        return 1
      if (
        a.album_name &&
      ...
      if (
        ...
        a.album_track_number > a.album_track_number &&
    ...
    // Sorting by Song ID
    return songs.sort((a, b) => {
      if (String(a.song_id).toLowerCase() > String(b.song_id).toLowerCase())
        return 1
      if (String(a.song_id).toLowerCase() < String(b.song_id).toLowerCase())
        return -1
      return 0
