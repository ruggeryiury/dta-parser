...
const stringifyDefault = (value: DTAFile, options: StringifyDataOptions): string => {
  ...
  output += `${genTabs()}('tuning_offset_cents' ${tuning_offset_cents === undefined ? 0 : tuning_offset_cents})${genTabs()}('guide_pitch_volume' ${
  ...
  })${genTabs()}('game_origin' '${gameOriginAsRB3DLC !== undefined && gameOriginAsRB3DLC ? 'rb3_dlc' : game_origin === undefined ? 'ugc_plus' : game_origin}')${genTabs()}('encoding' '${
    ...
    pack_name ? `${genTabs(1)}(pack_name "${quoteToSlashQ(pack_name)}")` : ''
...
const stringifyRB3DLC = (value: DTAFile, options: StringifyDataOptions): string => {
  ...
  if (fancyPanVolCores) {
    ...
    }${bass > 0 ? `      ${fancyPanVolRB3StringGenerator('bass', 'desc', panVol, guitarCores)}` : ''}${
      guitar > 0 ? `      ${fancyPanVolRB3StringGenerator('guitar', 'desc', panVol, guitarCores)}` : ''
    ...
    }${bass > 0 ? `      ${fancyPanVolRB3StringGenerator('bass', 'pans', panVol, guitarCores)}` : ''}${
    ...
    }${bass > 0 ? `      ${fancyPanVolRB3StringGenerator('bass', 'vols', panVol, guitarCores)}` : ''}${
    ...
    }${bass > 0 ? `      ${fancyPanVolRB3StringGenerator('bass', 'cores', panVol, guitarCores)}` : ''}${
  ...
  output += `${genTabs(2)}(band ${rank_band})${genTabs()})${genTabs()}(format ${format === undefined ? 10 : format})${genTabs()}(version ${
    version === undefined ? 30 : version
  })${genTabs()}(game_origin ${gameOriginAsRB3DLC !== undefined && gameOriginAsRB3DLC ? 'rb3_dlc' : game_origin === undefined ? 'ugc_plus' : game_origin})${genTabs()}(rating ${
    rating === undefined ? 4 : rating
  })${genTabs()}(genre ${genre === undefined ? 'other' : genre})${genTabs()}(sub_genre ${
    sub_genre === undefined ? 'subgenre_other' : sub_genre
  })${genTabs()}(vocal_gender ${vocal_gender})${genTabs()}(year_released ${year_released})${year_recorded ? `${genTabs()}(year_recorded ${year_recorded})` : ``}${genTabs()}(album_art ${
    album_art ? 'TRUE' : 'FALSE'
  })${album_name ? `${genTabs()}(album_name "${quoteToSlashQ(album_name)}")` : ``}${album_track_number ? `${genTabs()}(album_track_number ${album_track_number})` : ``}${
    vocal_tonic_note === undefined ? `` : `${genTabs()}(vocal_tonic_note ${vocal_tonic_note})`
  }${song_tonality === undefined ? `` : `${genTabs()}(song_tonality ${song_tonality})`}${song_key === undefined ? `` : `${genTabs()}(song_key ${song_key})`}${
    ...
    real_guitar_tuning ? `${genTabs()}(real_guitar_tuning (${real_guitar_tuning.join(' ')}))` : ``
  }${real_bass_tuning ? `${genTabs()}(real_bass_tuning (${real_bass_tuning.join(' ')}))` : ``}${encoding === undefined || encoding === 'latin1' ? `` : `${genTabs()}(encoding ${encoding})`}${
    pack_name ? `${genTabs()}(pack_name "${quoteToSlashQ(pack_name)}")` : ``
