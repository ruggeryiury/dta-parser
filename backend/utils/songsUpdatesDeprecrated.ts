...
const readSongsUpdates = (): SongsUpdatesObject => {
  ...
  const updateFileContents = fs.readFileSync(path.resolve('C:/Users/Ruggery/Documents/Visual Studio Code/Projects/dta-parser/backend/bin/database/songs_updates.dta'), 'utf-8')
  ...
  allSongsNames.forEach((songname) => {
    ...
    allKeys.forEach((keys) => {
      if (keys === 'vocal_parts' || keys === 'vols' || keys === 'hopo_threshold') {
        ...
        newSongsUpdates += `\n\t(${keys} "${(allSongsUpdates[songname][keys as keyof SongsUpdatesObject[typeof songname]] as string).replaceAll('"', '\\q')}")`
        ...
        newSongsUpdates += `\n\t(${keys} ${(allSongsUpdates[songname][keys as keyof SongsUpdatesObject[typeof songname]] as boolean) ? 'TRUE' : 'FALSE'})`
      } else {
